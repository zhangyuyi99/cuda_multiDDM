...
result_path='/cicutagroup/yz655/cuda_run_result/fps10_nott_videos_results/'          
for filename in ${fps10_nott_videos_path}*.avi; do
    # name=${filename:34:12}
    ...
    video_frame_number=$(ffprobe -select_streams v -show_streams ${filename} 2>/dev/null | grep nb_frames | sed -e 's/nb_frames=//')
    chunk_frame_number_ini=$((${video_frame_number}/3))
    # echo "${chunk_frame_number}"

    chunk_frame_number=$((chunk_frame_number_ini>100 ? 100 : chunk_frame_number_ini))
    ...
    ./main -o ${output_path} -f ${filename} -N ${video_frame_number} -C ${chunk_frame_number} -T '/cicutagroup/yz655/cuda_run/tau.txt' -S '/cicutagroup/yz655/cuda_run/scale.txt' -Q '/cicutagroup/yz655/cuda_run/lambda.txt'
